...
def get_customer_reqirements(customer_msg):
    system_msg = "Given the following conversation and a follow-up question, what is the customer's budget, preferred CPU brand, preferred GPU brand, and minimum storage size? Reply ONLY in this FORMAT: [\"budget\", \"CPU brand\", \"GPU brand\", \"minimum storage\", \"liquid/air\" (cooling)], and use \"\" for fields that the customer did not specify. "
    
    human_msg = f"Customer's Question: {customer_msg}\nFORMATTED answer: "
    # send requirements to gpt-3.5
    response = send_request_openai("gpt-3.5-turbo", system_msg, human_msg, 0, 256)
...
def product_retrieve(customer_msg, budget_msg, get_comment):
    if 'budget' in customer_msg:
        # manully remove the 'within' and 'under' from the message if it is about budget
        customer_msg = customer_msg.replace("within", "of").replace("under", "of")
    
    print("Doing product query, customer_msg = {}".format(customer_msg))
    quary1 = q1_prompt + customer_msg + " budget<=" + budget_msg # under budget product retrieve
    quary2 = q2_prompt + customer_msg + " budget>=" + budget_msg # over  budget product retrieve
    ...
    if res1 != "nothing" and not res1_check_failed:
        ...
        prod1 = "NO PRODUCT UNDER THE BUDGET! You can offer the 10% coupon mentioned above. "
...
def find_suitable_builds(df, reqirement_list):
    budget, cpu, gpu, storage, cooling = reqirement_list
    ...
    # filtered by cooling system
    if cooling:
        if cooling == 'liquid':
            filtered_df = filtered_df[filtered_df['Processor Cooling (CPU Fan / CPU Cooling / Cooling System)'].str.contains('liquid', case=False)]
        else:
            # look for air cooling and fan
            filtered_df = filtered_df[filtered_df['Processor Cooling (CPU Fan / CPU Cooling / Cooling System)'].str.contains(' Air|Fan', case=False)]
...
def product_retrieve_gpt3_5(customer_msg, budget_msg, get_comment):
    # returned empty list if failed
    ...
    if len(reqirement_list) == 0:
        ...
        product_info = product_retrieve(customer_msg, budget_msg, get_comment)
        return product_info
    elif len(reqirement_list) == 5:
        # if gpt3.5 works, find out the product
        under_index, over_index = find_suitable_builds(df, reqirement_list)
